#!/bin/bash
# OAuth2 Device Flow for NVIDIA SSO with new client credentials
# Based on RFC 8628 with NVIDIA Starfleet extensions

CLIENT_ID="uhwdRbTjcQ6UCdu6NdxiXStrR3m1OX2CfENbAc_M-R0"
CLIENT_SECRET="sfcs-XapbPgMmYKupr0PG6iS0gLjzgGzX1mvx"
...
while true; do
    ...
    if echo "$TOKEN_RESPONSE" | jq -e '.access_token' > /dev/null 2>&1; then
        ...
        EMAIL=$(echo "$USER_INFO" | jq -r '.email // "N/A"' 2>/dev/null)
        echo ""
        ...
        echo ""
        echo "Test with Talk-to-Data:"
        echo "  curl -H \"Authorization: Bearer \$ACCESS_TOKEN\" \\"
        echo "       -H \"Content-Type: application/json\" \\"
        echo "       -d '{\"input_message\": \"What was GTC count?\"}' \\"
    ...
    elif echo "$TOKEN_RESPONSE" | jq -e '.error' | grep -q "access_denied"; then
        echo ""
